...
void gsMenuCreate(void) {
	...
	paletteLoad("data/_old/palettes/selur_v1.plt", s_pMenuVPort->pPalette, 32);

	cursorCreate(s_pMenuView, 0, "data/_old/hand.bm", 0);

	s_pGrassBitMap = bitmapCreateFromFile("data/_old/bitmaps/grass.bm", FALSE);

	// Splash
	s_pSplashBitMap = bitmapCreateFromFile("data/_old/bitmaps/splash.bm", FALSE);
